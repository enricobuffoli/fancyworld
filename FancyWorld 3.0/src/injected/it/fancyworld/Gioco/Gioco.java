...
import injected.it.fancyworld.Gioco.Luoghi.LuogoBloccato;
...
public class Gioco extends Thread
{
    ...
    private Gioco()
    {
        ...
        else
        {
            ...
            {
                System.out.println("Vuoi caricare i dati precedentemento salvati? (Y/N)");
                String input = Utility.generalKeyInput();
                if(input.equals("Y"))
                {
        ...
        while(true)
        {
            ...
            if(!(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()) instanceof Start) && !(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()) instanceof Goal))
            {
                if(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()).getChiaveDelLuogo() == null)
                {
                    ...
                    if(giocatore.getChiavi().size() != 0)
                    {
                        System.out.println("Vuoi depositare una delle chiavi di cui sei in possesso? ");
                        String temp = Utility.yN();
                        if(temp.equals("Y"))
                        {
                            System.out.println("Sei in possesso delle seguenti chiavi:");
                ...
                else
                {
                    ...
                    if(temp.equals("Y"))
                    {
                        ...
                        if ((giocatore.getChiavi().size() + 1) <= 5)
                        {
