...
public class Livello
{
    ...
    {
        System.out. println("Indice del livello: " + levelIndex);
        if(levelIndex != 10)
            for(UpStairs temp : upStairses)
                placesConnectionPath(temp);
        else
        {
            System.out.println("X: " + goal.getPosition().x + " Y: " + goal.getPosition().y);
    ...
    private void placesConnectionPath(Luogo temp)
    {
        ...
        if(x < 0)
            ...
            generalConnectionPathX(startPos.x ,temp.getPosition().x);

        if(y < 0)
            generalConnectionPathY(temp.getPosition().y, startPos.y, temp.getPosition().x);
        else
            generalConnectionPathY(startPos.y, temp.getPosition().y, temp.getPosition().x);
    }
    ...
    private void generalConnectionPathY(int yIn ,int yFin, int xStart)
    {
        for(int i = yIn; i <= yFin; i++)
        {
            if(riga.get(xStart).get(i).getTag().equals("N"))
                riga.get(xStart).set(i, new Pass(xStart, i, getPlaceName()));
        }
    }
...
//                    map += (char)0 + "" + (char)0;
                    ...
                    if(riga.get(i).get(j) instanceof Trap)
                    	map += "T";
                    /*if (riga.get(i).get(j).isPassed() && riga.get(i).get(j) instanceof UpStairs)*/
                    else if(riga.get(i).get(j) instanceof UpStairs)
                        map += "U";
                    /*if (riga.get(i).get(j).isPassed() && riga.get(i).get(j) instanceof DownStairs)*/
                     else if(riga.get(i).get(j) instanceof DownStairs)
                        map += "D";
                    else if(riga.get(i).get(j) instanceof Pass)
                        map += "P";
                    else if(riga.get(i).get(j) instanceof Goal)
                        map += "G";
                	else if (riga.get(i).get(j).isPassed())
                        map += " ";
                    else
                    	map +="N";
//                        map += "██";
