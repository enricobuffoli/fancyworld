...
import injected.it.fancyworld.Gioco.Luoghi.Goal;
import injected.it.fancyworld.Gioco.Luoghi.LuoghiConChiave.LuogoBloccato;
import injected.it.fancyworld.Gioco.Luoghi.Start;
    ...
    public void run()
    {
        while(true)
        {
            ...
            else if(world.getCurrentLevel().isPossibleGoesThisWay(way, giocatore.getPosition()) == 1)
            {
                ...
                System.out.println("X giocatore: " + giocatore.getPosition().x + " Y giocatore: " + giocatore.getPosition().y);
                ...
                LuogoBloccato lockedPlace = (LuogoBloccato) world.getCurrentLevel().getRiga().get(x).get(y);
                ...
                {
                    System.out.println("Il giocatore è in possesso della seguente chiave: chiave di " + giocatore.getChiave().toString() + "\nVuoi tentare di aprire la porta? (Y/N)");
                    String temp = Utility.yN();
                    if(temp.equals("Y"))
            ...
            if(!(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()) instanceof Start) && !(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()) instanceof Goal))
            {
                if(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()).getChiaveDelLuogo() == null)
                {
                    ...
                    if(giocatore.getChiave() != null)
                    {
                        ...
                        System.out.println("Non hai chiavi da depositare");
                }
                else
                {
                    ...
                    String temp = Utility.yN();
                    if(temp.equals("Y"))
                        giocatore.setChiave(world.getCurrentLevel().getCurrentPlace(giocatore.getPosition()).pickUpKey());
                }
            }
