...
import injected.it.fancyworld.Gioco.Prove.ProvaDifficile;
import injected.it.fancyworld.Gioco.Prove.ProvaFacile;
import injected.it.fancyworld.Gioco.Prove.ProvaIntermedia;
...
public class Livello implements Serializable
{
    ...
    private int levelIndex,levUp,levTrap, levDoor, levTrial;
    ...
    public Livello(int levelIndex, Point startPos, ArrayList<Luogo> downStairs, Point startUp)
    {
        ...
        traps = new ArrayList<>();
        levTrial = Utility.getRandomBetween2(10) + 1;
        createLevel();
    }
    ...
    private void createLevel()
    {
        ...
        System.out.println("levTrial: " + levTrial);

        for(int i = 0; i < levTrial; i++)
        {
            int x = Utility.getRandomNumber();
            int y = Utility.getRandomNumber();
            Prova.TipoProva tipo = Utility.getRandomTipoProva();
            //System.out.println("X: " + x + " Y: " + y + " Tipo: " + tipo);

            if(riga.get(x).get(y).getProva() != null || riga.get(x).get(y).getTipoLuogo().equals(TipoLuoghi.NORMAL) || riga.get(x).get(y).getTipoLuogo().equals(TipoLuoghi.START) || riga.get(x).get(y).getTipoLuogo().equals(TipoLuoghi.GOAL))
                i--;
            else
            {
                if(tipo.equals(Prova.TipoProva.EASY))
                    riga.get(x).get(y).setProva(new ProvaFacile());
                else if(tipo.equals(Prova.TipoProva.MEDIUM))
                    riga.get(x).get(y).setProva(new ProvaIntermedia());
                else
                    riga.get(x).get(y).setProva(new ProvaDifficile());
            }
        }
    }
